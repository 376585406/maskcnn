...
def load_pretrained_ckpt(net, load_path):
    """
...
def train_maskrcnn():
    ...
    dataset = create_coco_dataset(mindrecord_file, batch_size=config.batch_size, device_num=device_num, rank_id=rank).project(["image", "image_shape","box", "label","valid_num", "mask"])
    ...
    if load_path != "":
        ...
        net = load_pretrained_ckpt(net=net, load_path=load_path)
